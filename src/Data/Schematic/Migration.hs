...
data Diff = DiffSchema [PathSegment] Action
...
data Versioned = Versioned
  Schema      -- base version
  [Migration] -- a bunch of migrations
...
type family Typed (s :: Schema) (ps :: PathSegment) :: Constraint where
  Typed ('SchemaObject '[]) ('Key fn)               = FalseConstraint fn
  Typed ('SchemaObject ( '(fn, s) ': tl)) ('Key fn) = ()
